...
``` c++
// 链式存储的二叉树，详细代码见我的仓库中,包括三个函数insert插入，find查找，draw画出树
```

其中值得提到的就是画出二叉树draw

```c
// 画出向左旋转90度的二叉树本质上是一个类中序遍历
// 其代码结构为，即只对根节点进行处理，然后对左右节点进行遍历，这三个的顺序根据遍历顺序排列即可
/* 伪代码：
fuc(root)
{
	fuc(root->r);
...
char 型一般不用来作为函数参数传递和返回，一般用int类型代替。
